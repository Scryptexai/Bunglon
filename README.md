...
`tests/test_project_contract.py` memeriksa scene entry point, node contract, asset path, semua ability, uniqueness class, validitas WAV, serta boundary controller/combat Phase 7–9. Lima smoke Godot menjalankan import/runtime nyata untuk GLB, `AnimationPlayer`/`AnimationTree`, helper, LOD, jalur player/AI, command desktop/touch/AI/external, basic attack/projectile/Hurtbox, dan lifecycle/timing Q/E/R/F. Hasil dan batasan engine tercatat di [`PHASE_6_GODOT_INTEGRATION.md`](heroes/hero_agile_hunter/docs/PHASE_6_GODOT_INTEGRATION.md), [`PHASE_7_CONTROLLER_INTEGRATION.md`](heroes/hero_agile_hunter/docs/PHASE_7_CONTROLLER_INTEGRATION.md), [`PHASE_8_BASIC_ATTACK_INTEGRATION.md`](heroes/hero_agile_hunter/docs/PHASE_8_BASIC_ATTACK_INTEGRATION.md), dan [`PHASE_9_ABILITY_TIMING.md`](heroes/hero_agile_hunter/docs/PHASE_9_ABILITY_TIMING.md).

## Status terhadap 3D Hero Production Roadmap V1

Roadmap menetapkan bahwa primitive/blocky mesh tidak boleh menjadi final character. **Phase 2 — 3D Character** menyediakan mesh `.glb` nyata di `character_final/`; **Phase 3 — Topology & Game Readiness** menyediakan LOD0/LOD1 yang dioptimalkan, UV atlas, material PBR, dan normal/tangent MikkTSpace; **Phase 4 — Rigging & Skinning** menyediakan GLB ber-skeleton, skin weights, inverse-bind matrices, socket helpers, serta bukti deformasi; **Phase 5 — Authored Animation Action Set** menyediakan 23 action GLB nyata dengan 523 channel/sampler rotasi dan semantic timing contract di `character_animated/`; **Phase 6 — Godot Integration** memakai asset tersebut sebagai visual aktif melalui adapter terpisah, `AnimationTree`, imported helper sockets, semantic presentation events, serta LOD swap tervalidasi; **Phase 7 — Controller & Command Integration** memvalidasi jalur player/touch/AI/external; **Phase 8 — Basic Attack & Projectile Authority** memvalidasi target hostile/range, satu projectile dari imported socket, Hurtbox/DamageReceiver, recovery, serta cancellation; dan **Phase 9 — Ability Lifecycle & Authored Timing** memvalidasi Q/E/R/F terhadap timing manifest sambil mempertahankan timer damage/dash/status di gameplay. Tidak ada jalur visual primitive/prosedural yang aktif.
...
- [`heroes/hero_agile_hunter/docs/PHASE_8_BASIC_ATTACK_INTEGRATION.md`](heroes/hero_agile_hunter/docs/PHASE_8_BASIC_ATTACK_INTEGRATION.md) — hostile/range gate, authoritative projectile, imported socket placement, damage, and cancellation evidence.
- [`heroes/hero_agile_hunter/docs/PHASE_9_ABILITY_TIMING.md`](heroes/hero_agile_hunter/docs/PHASE_9_ABILITY_TIMING.md) — Q/E/R/F cast/action/recovery timing, energy/status/lock, interruption, and engine evidence.
