...
This directory remains the Phase 5 animation asset handoff. **Phase 6** has now completed real Godot 4.3 import, `AnimationPlayer` / `AnimationTree` mapping, runtime helper resolution, safe semantic-event dispatch, imported projectile/VFX attachment points, and player/AI presentation integration; see [`../heroes/hero_agile_hunter/docs/PHASE_6_GODOT_INTEGRATION.md`](../heroes/hero_agile_hunter/docs/PHASE_6_GODOT_INTEGRATION.md). **Later phases** still own broader combat gameplay validation, Android/mobile profiling, and final playtest sign-off. The event data remains a timing contract for the adapter, never visual authority for collision or damage.
